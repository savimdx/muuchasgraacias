...
import { redirectWithParams } from '../utils/redirect';
import { 
  ShieldCheck, 
  BookOpen, 
  Award, 
...
export default function UpsellRFEF({ onAccept, onDecline }: UpsellRFEFProps) {
  const handleAccept = () => {
    if (onAccept) {
      ...
      redirectWithParams("https://pay.hotmart.com/P106371037H?off=upsell_accepted");
    }
  };
  ...
  const handleDecline = () => {
    if (onDecline) {
      ...
      redirectWithParams("https://pay.hotmart.com/P106371037H?off=upsell_declined");
    }
  };
  return (
